 ...
 */
export async function getTrendDrillCacheWorkItems(params, entityType, cacheID, category, deletedItemsCategory) {
    let entityTypeQueryValue = (entityType === 'backlog_items' ? `;(subtype IN 'story','defect','quality_story');` : `;(subtype='${entityType}')`);
    const response = await fetch(`${params.octane_url}/api/shared_spaces/${params.shared_space}/workspaces/${params.workspace}/work_items?fields=id,name,is_deleted_entity,trend_drill_down_field_old_values,trend_drill_down_field_new_values,phase,actual_story_points,initial_estimate,story_points,estimated_hours,invested_hours,remaining_hours&limit=2000&offset=0&order_by=id&query="((trend_drill_cache_data={((trend_drill_cache={(id='${cacheID}')});(category IN ${category}))})${entityTypeQueryValue})"&trend_drill_down_cache_data_category=${deletedItemsCategory}&trend_drill_down_cache_id=${cacheID}`);
    const data = await response.json();
    return data;
}
...
export async function getItems(params, entityName, releaseID, releaseSpecific){
    ...
    if (entityVal) {
        ...
        if (releaseID) {
            ...
            if (releaseSpecific) {
                query += ';(release_specific=true);activity_level=0';
            }
            query += `)"`;
        }
